...
test: ## run tests quickly with the default Python
	python -m pytest --basetemp=${ENVTMPDIR} --cov=metad --cov-report xml

.PHONY: lint
lint: ## check style with flake8 and isort
	flake8 metad tests
	isort -c --recursive metad tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find metad tests -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive metad tests
	isort --apply --atomic --recursive metad tests

.PHONY: coverage
coverage: ## check code coverage quickly with the default Python
	coverage run --source metad -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --separate --no-toc -o docs/api/ metad
	$(MAKE) -C docs html
