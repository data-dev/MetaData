...
MetaData could always use more documentation, whether as part of the
official MetaData docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `MetaData` for local development.

1. Fork the `MetaData` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/MetaData.git
...
2. The tests that cover a module called ``metad/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/metad/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_metad
...
2. Update the version in ``setup.cfg``, ``metad/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``metad/__init__.py``
...
or in command line::
    ...
    pip install 'metad>=X.Y.Z.dev'


.. _GitHub issues page: https://github.com/data-dev/MetaData/issues
.. _Build Status page: https://github.com/data-dev/MetaData/actions
